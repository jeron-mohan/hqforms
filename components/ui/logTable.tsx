...
export function LogTable({ finalData }: { finalData: FormData[] }) {
  ...
  useEffect(() => {
    const parsedForms = finalData.map(form => {
      let parsedJsonForm;
      try {
        parsedJsonForm = JSON.parse(form.jsonForm || '{}');
      } catch (error) {
      ...
      return {
        ...form,
        formTitle: parsedJsonForm.formTitle || 'Untitled Form',
      };
    });
            ...
            <TableBody>
              {forms.slice(-1).concat(forms.slice(0, -1)).map((form, index) => (
                <TableRow key={index} className="cursor-pointer" onClick={() => handleRowClick(form.id)}>
                  ...
                  <TableCell className="hidden md:table-cell">
                    {form.createdAt instanceof Date ? form.createdAt.toLocaleString() : 'Invalid Date'}
                  </TableCell>
                  <TableCell>
                    <DropdownMenu>
